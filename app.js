...
// Each monitor can have its own independent layout and name.
let appData = {
    settings: {
        // Global color defaults (can be overridden by layout styles)
        color_bg: '#0f0f11',
    ...
    monitors: [
        {
            ...
            layout: 'layout-1-sidebar-grid', // The default layout for this specific monitor
            categories: [
                { 
        ...
        },
        {
            ...
            layout: 'layout-4-masonry-hybrid',
            categories: [
                { 
...
// Internal reference copy of original dummy data (for resetting)
const dummyDataCopy = JSON.parse(JSON.stringify(appData));
...
function init() {
    ...
    if (saved) {
        // Only load saved data if it exists. Otherwise, fall back to the pre-populated default defined above.
...
// Global System Settings (Background, Main colors, Font)
...
function updateMonitorDropdowns() {
    ...
    // Handle monitor specific layout dropdown setting
    if(appData.monitors.length > 0) {
        document.getElementById('layout-select').value = appData.monitors[currentMonitorIndex].layout || 'layout-1-sidebar-grid';
    }
}
...
document.getElementById('cms-monitor-select').addEventListener('change', (e) => {
    ...
    updateMonitorDropdowns(); // Update layout selector too
    renderCms();
});
document.getElementById('active-monitor-select').addEventListener('change', (e) => {
    ...
    updateMonitorDropdowns(); // Sync layout dropdown in background
    renderDisplay();
});

// Settings Handlers (Global Color/Font)
['color-bg', 'color-primary', 'color-secondary', 'font-select'].forEach(id => {
...
// Independent Monitor Layout Handler
document.getElementById('layout-select').addEventListener('change', (e) => {
    // Save the selection only to the current active monitor
    appData.monitors[currentMonitorIndex].layout = e.target.value;
    // applyIndependentMonitorSettings(appData.monitors[currentMonitorIndex]);
});
    ...
    currentMonitor.categories.forEach((cat, catIndex) => {
        ...
        catDiv.innerHTML = `
            <div class="category-header">
                ...
                <button onclick="deleteCategory(${catIndex})" class="danger-btn" style="width:auto; margin:0;">Delete Grp</button>
            </div>
...
window.addDish = (cIdx) => {
    appData.monitors[currentMonitorIndex].categories[cIdx].items.push({ name_ru: 'Новое блюдо', name_kk: 'Жаңа тағам', price: '', image: '', vegan: false, beef: false, chicken: false, isNew: false });
    renderCms();
};
...
window.deleteCategory = (cIdx) => {
    if(confirm('Are you sure you want to delete this entire group and all its dishes?')){
        appData.monitors[currentMonitorIndex].categories.splice(cIdx, 1);
        renderCms();
    }
}
...
document.getElementById('add-monitor-btn').addEventListener('click', () => {
    ...
    appData.monitors.push({ id: newId, name: `Monitor ${newId}`, layout: 'layout-1-sidebar-grid', categories: [] });
...
window.handleImageUpload = (event, catIndex, itemIndex) => {
    ...
    if (file) {
        if (file.size > 800 * 1024) { alert('Image too large (>800KB). Please compress images before upload to avoid storage limits.'); return; }
...
document.getElementById('save-data-btn').addEventListener('click', () => {
    ...
    } catch (e) { alert('Storage limit reached! Please use smaller images (<800KB) and refresh.'); console.error(e); }
});
document.getElementById('reset-data-btn').addEventListener('click', () => {
    if(confirm('This will wipe all your custom changes and restore the default dummy menu. Continue?')){
        localStorage.removeItem('fastfoodMenuData');
        location.reload();
    }
});
...
function renderDisplay() {
    const currentMonitor = appData.monitors[currentMonitorIndex];
    if(!currentMonitor) { containers.board.innerHTML = '<h2>No Monitor Found</h2>'; return; }

    // Apply the independent layout class *only* for this monitor display
